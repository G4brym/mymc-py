...
# ps2save is only needed for PSU/MAX save-file import/export, which is not
# part of this port.  Keep the import optional so the core filesystem works
# standalone.
try:
    import ps2save  # type: ignore
except ImportError:
    ps2save = None
...
class ps2mc(object):
	...
	def import_save_file(self, sf, ignore_existing, dirname = None):
		...
		if ps2save is None:
			raise NotImplementedError(
				"save-file import (.psu/.max/.cbs) requires the "
				"ps2save module, which is not bundled with mymc-py. "
				"Use add/mkdir with folder-style saves instead.")
	...
	def export_save_file(self, filename):
		if ps2save is None:
			raise NotImplementedError(
				"save-file export (.psu/.max) requires the "
				"ps2save module, which is not bundled with "
				"mymc-py. Use extract instead.")
	...
	def _check_dir(self, fat, dirloc, dirname, ent):
		...
		if why != None:
			print("bad directory:", dirname + b":", why)
			return False
		ret = True
		first_cluster = ent[4]
		length = ent[2]
		...
		dot_ent = dir[0]
		if dot_ent[8] != b".":
			print("bad directory:", dirname + b': missing "." entry')
			ret = False
		if (dot_ent[4], dot_ent[5]) != dirloc:
			print("bad directory:", dirname + b': bad "." entry')
			ret = False
		if dir[1][8] != b"..":
